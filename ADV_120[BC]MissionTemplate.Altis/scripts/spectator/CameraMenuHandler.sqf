...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if(KEGs_camSelLast != lbCurSel KEGs_cLBCameras) then 
	{
		...
		if (_KEGs_cs isEqualTo 3) then {[] spawn BIS_fnc_Camera;_cam = missionnamespace getvariable ["BIS_fnc_camera_cam",objnull];    _cam setPosATL (KEGs_target modelToWorld [0,-50,((getPosATL _cam) select 2)]);_cam setDir (getDir KEGs_target);[_cam,KEGscam_free_pitch,0] call bis_fnc_setpitchbank;};
